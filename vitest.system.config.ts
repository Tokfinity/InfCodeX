...
// filesystem handles. Keep limited parallelism so production timeout contracts
// are measured against the code rather than worker starvation.
export default mergeConfig(
  createVitestConfig({ include: SYSTEM_TEST_FILES, exclude: [] }),
  defineConfig({ test: { maxWorkers: 2 } }),
);
