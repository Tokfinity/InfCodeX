...
| 088 | High | Resolved | 消息列表视口布局不稳定 - 底部区域跳动/最后一行被裁剪 | v0.5.39 | 2026-03-16 |
| 087 | Medium | Resolved | 自动补全触发冲突 - @文件路径中/错误触发命令补全 | v0.5.33 | 2026-03-13 |
| 086 | High | Resolved | 自动补全竞态条件导致快速输入时显示过期补全 | v0.5.32 | 2026-03-12 |
| 085 | Medium | Resolved | 只读 Bash 命令白名单未在非 plan 模式复用 | v0.5.30 | 2026-03-12 |
| 084 | High | Resolved | 流式响应长时间静默中断无任何提示 | v0.5.30 | 2026-03-12 |
| 013 | Low | Resolved | 自动补全缓存内存泄漏风险 | v0.6.17 | 2026-03-23 |
| 014 | Low | Resolved | 语法高亮语言支持不全 | v0.6.17 | 2026-03-23 |
| 015 | Low | Resolved | Unicode 检测不完整 | v0.6.17 | 2026-03-23 |
| 017 | Low | Resolved | TextBuffer 未使用方法 | v0.6.17 | 2026-03-23 |
| 018 | Low | Resolved | TODO 注释未清理 | v0.6.17 | 2026-03-23 |
| 055 | Low | Resolved | Built-in Skills 未完全符合 Agent Skills 规范 | v0.6.17 | 2026-03-23 |
| 061 | Low | Resolved | Windows Terminal 流式输出时滚轮滚动异常 | v0.6.17 | 2026-03-23 |
| 077 | Low | Resolved | Skills 系统高级功能未完全实现 | v0.6.17 | 2026-03-23 |
| 083 | Medium | Resolved | 缺少快捷键系统 | v0.5.30 | 2026-03-12 |
| 089 | High | Resolved | Feature / Design / Summary 元数据漂移 | v0.6.10 | 2026-03-19 |
| 090 | High | Resolved | CLI Provider 桥接语义降级：上下文与 MCP 能力丢失 | v0.6.10 | 2026-03-19 |
| 100 | High | Resolved | ACP Server 缺少日志/可观测性输出 | v0.6.15 | 2026-03-23 |
| 101 | High | Resolved | Adaptive multi-agent code review loses Generator output and gives Evaluator a truncated handoff | v0.7.5 | 2026-03-27 |
| 102 | Medium | Resolved | Repo-intelligence mixes git-tracked and filesystem-discovered file sets | v0.7.5 | 2026-03-28 |
| 103 | Low | Resolved | Managed-task planning recomputes repo routing signals in the same workspace | v0.7.5 | 2026-03-28 |
| 104 | Low | Resolved | Repo-intelligence cache JSON is read without runtime shape validation | v0.7.5 | 2026-03-28 |
| 108 | High | Resolved | ACP server 链路未接入 MCP — 编辑器/ACP 场景下 mcpServers 配置不生效 | v0.7.35 | 2026-06-11 |
| 109 | Low | Resolved | 缺少 mcp_get_prompt 工具 — MCP prompt 能力未暴露给模型 | v0.7.35 | 2026-06-11 |
| 111 | Low | Resolved | SSE / Streamable HTTP MCP 传输缺少专项测试 | v0.7.48 | 2026-06-11 |
| 113 | High | Resolved | Ctrl+C 中断后工具调用仍继续执行 — abort signal 未传播到工具执行阶段 | v0.7.18 | 2026-04-12 |
| 114 | High | Resolved | ask_user_question ESC 取消被静默吞掉 — 用户取消后模型继续执行 | v0.7.18 | 2026-04-12 |
| 115 | Medium | Resolved | Managed foreground ledger 切换 kind 时丢失 tool_group 引用 — 工具历史条目重复 | v0.7.18 | 2026-04-13 |
| 116 | Medium | Resolved | 协议代码块泄漏到用户可见文本 + H0_DIRECT routing 诊断噪音 | v0.7.18 | 2026-04-13 |
| 117 | High | Resolved | Managed task error recovery 断裂 — transient error 后任务未恢复 | v0.7.18 | 2026-04-13 |
| 121 | High | Resolved | 超长粘贴（>500 字符）导致用户 prompt 在历史中视觉假消失 + 粘贴后 500ms+ 键击延迟 | v0.7.24 | 2026-04-20 |
| 123 | High | Resolved | Win10 远端 OpenSSH/ConPTY 下 kodax 全屏闪烁 — altScreen 分支 log.clear()+log() 拆成两次独立 stdout.write | v0.7.27 | 2026-04-24 |
| 127 | High | Resolved | AMA 每次 query 都弹出"发现未完成的任务"对话框 — managed task checkpoint 清理 race + 错误/取消路径未清理 | v0.7.34 | 2026-05-03 |
| 128 | Medium | Resolved | runKodaX 端到端 contract 测试在并发负载下 5000ms 超时偶发 flake | v0.7.34 | 2026-05-03 |
| 129 | Medium | Resolved | Auto 模式下纯只读管道命令被误判为"修改文件"并强制确认 — `2>NUL` stderr 重定向 + 缺 `findstr` 白名单 + 管道一票否决 | v0.7.38 | 2026-05-09 |
| 130 | Medium | Resolved | Managed foreground ledger phase/iteration 切换时丢失 in-flight tool_group 引用 — Issue 115 对称路径漏覆盖 | v0.7.39 | 2026-05-12 |
| 131 | Medium | Resolved | Auto[llm] 模式 Windows cmd flag (`/R` / `/B` / `/Y` / `/MIR` 等) 被 `looksLikePath` 误识别为 POSIX 绝对路径 → "Protected path" 假阳性 confirm | v0.7.39 | 2026-05-12 |
| 132 | Low | Resolved | `h2-boundary-runner.test.ts` "session.jsonl" ENOENT — Windows fs visibility race + silent error swallow in persistCell | v0.7.41 | 2026-05-16 |
| 134 | High | Resolved | Plan list shows `0/N completed` mid-task when Worker re-calls `op:'init'` — `todoStore.init()` unconditional reset wipes completed/skipped/cancelled status | v0.7.42 | 2026-05-19 |
| 135 | Medium | Resolved | FEATURE_167 B2 synth verdict path bypasses `autoCompleteOnAccept` — UI shows `0/N completed` even when run terminates as `accept` | v0.7.42 | 2026-05-19 |
| 137 | High | Resolved | Streamable HTTP MCP transport drops `Mcp-Session-Id` on sessionful servers | v0.7.45 | 2026-06-05 |
| 006 | Low | Resolved | 整数解析无范围检查 | v0.5.29 | 2026-03-11 |
| 046 | High | Resolved | Session 恢复时消息显示异常 | v0.4.5 | 2026-02-27 |
| 047 | Medium | Resolved | 流式输出时界面闪烁 | v0.4.5 | 2026-02-27 |
| 048 | Medium | Resolved | Spinner 动画期间消息显示乱序 | v0.4.5 | 2026-02-27 |
| 049 | High | Resolved | 权限模式持久化位置错误 | v0.5.0 | 2026-02-27 |
| 050 | Medium | Resolved | 命令输出格式不一致（AI 编造问题） | v0.4.6 | 2026-02-28 |
| 051 | Medium | Resolved | 权限确认取消时无提示 | v0.4.6 | 2026-02-28 |
| 052 | High | Resolved | 受保护路径确认对话框显示错误选项 | v0.4.6 | 2026-02-28 |
| 054 | Medium | Resolved | Agent Skills 系统未与 LLM 集成 | v0.5.5 | 2026-03-04 |
| 056 | Medium | Resolved | Skills 系统缺少渐进式披露机制 | v0.4.8 | 2026-03-01 |
| 057 | Medium | Resolved | Skill 命令格式不符合 pi-mono 设计规范 | v0.4.8 | 2026-03-01 |
| 059 | High | Resolved | VS Code Terminal UI 重复渲染问题 | - | 2026-03-11 |
| 060 | Medium | Resolved | 上下文使用状态栏显示 | v0.5.12 | 2026-03-07 |
| 062 | High | Resolved | 流式响应结束后 StatusBar 仍显示 Compacting | v0.5.16 | 2026-03-08 |
| 064 | High | Resolved | 技能单例重复创建问题 | v0.5.4 | 2026-03-03 |
| 065 | Medium | Resolved | Skills 系统缺少自然语言触发 | v0.5.5 | 2026-03-04 |
| 066 | High | Resolved | /project init 命令在 InkREPL 中静默失败 | v0.5.4 | 2026-03-03 |
| 067 | Critical | Resolved | API 速率限制重试机制失效 | v0.5.27 | 2026-03-11 |
| 068 | High | Resolved | Thinking 指示器长时间无进度反馈 | v0.5.4 | 2026-03-03 |
| 069 | Medium | Resolved | 缺少 LLM 交互式提问工具 | v0.5.29 | 2026-03-11 |
| 070 | Low | Resolved | 流式输出可能丢失换行符 | v0.5.29 | 2026-03-11 |
| 071 | High | Resolved | Session Resume 跨项目恢复错误 | v0.5.4 | 2026-03-03 |
| 072 | High | Resolved | 流式中断后 tool_call_id 不匹配导致 API 错误 | v0.5.20 | 2026-03-07 |
| 073 | High | Resolved | /project auto 等子命令无流式进度反馈 | v0.5.4 | 2026-03-03 |
| 074 | Medium | Resolved | 多轮迭代时 Thinking 和 Response 内容累积显示 | v0.5.7 | 2026-03-04 |
| 075 | Medium | Resolved | 粘贴多行文本到输入框时换行丢失 | v0.5.4 | 2026-03-03 |
| 076 | Medium | Resolved | 正常响应后历史记录偶现 [Interrupted] 标记 | v0.5.4 | 2026-03-04 |
| 078 | High | Resolved | CLI --max-iter 默认值覆盖 coding 包默认值 | v0.5.5 | 2026-03-04 |
...
- 实施 commits：`9a001bb` (slice 1 module + 22 tests) → `a39ff5d` (slice 1 review fix — flattenArgv 注释) → `f61720c` (slice 2 isBashRead/Write AST 切换 + 删 strip-then-classify) → `23dec91` (slice 2 review fix — bash-ast 注释) → `dc8b848` (slice 3 extractPaths / collectBashWriteTargets AST hybrid) → `4572758` (slice 3 review fix — 测试注释精度)。
