...
> **Current package / SDK state (2026-07-03 / v0.7.59)**: 源码 workspace 为 `llm / agent / coding / repl` 4 包；根 npm 包 `@kodax-ai/kodax` 暴露 8 个 SDK subpath（`/agent`、`/llm`、`/coding`、`/media`、`/repl`、`/skills`、`/mcp`、`/session`）；LLM registry 有 15 个内置 provider alias（含 `zai-coding`）。v0.7.58 的当前事实新增 inline workflow authoring（model-callable `run_workflow` 工具，ADR-044/046/047/048）、profile-gated SDK agent-profile 面（`KodaXAgentProfile`，FEATURE_247）、可白标的 self-knowledge manual（FEATURE_221）、config surface 扩展（model tiers + 原 env-only 设置进 `config.json` / `KodaXOptions`，per-run `AsyncLocalStorage` 隔离）；既有事实仍含 effort-first reasoning、`zai-coding` provider alias、内置 repo-intelligence semantic engine、`/media` SDK 入口，以及 workflow process / durable replay / `hostMetadata` / inline skill-reference propagation；v0.7.59 rollup 增加 AMAW mode-level orchestration directive（FEATURE_248）与 AMA natural-language workflow activation（FEATURE_249）。
>
> 之前的执行模型注脚（v0.7.42 前）：
