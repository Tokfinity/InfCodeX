# KodaX SDK 鈥?Embedder Integration Guide

> Audience: host applications embedding `@kodax-ai/kodax` (and its
> subpaths) as a substrate 鈥?e.g. KodaX Space's desktop wrapper, IDE
...
1. [MCP server management 鈥?`McpManager` runtime API](#1-mcp-server-management--mcpmanager-runtime-api)
2. [Skill `!`cmd`` dynamic-context resolution + `IVariableResolver`](#2-skill-cmd-dynamic-context-resolution--ivariableresolver)
3. [Per-app data directory namespacing 鈥?`getAppDataDir`](#3-per-app-data-directory-namespacing--getappdatadir)
...
6. [Session persistence 鈥?wiring `runKodaX` to disk](#6-session-persistence--wiring-runkodax-to-disk)
...
8. [User-authored agents 鈥?markdown loader + extension `registerAgent`](#8-user-authored-agents--markdown-loader--extension-registeragent-feature_191-v0743)
9. [Electron + `stdio: 'inherit'` on Windows 鈥?PowerShell input hijack](#9-electron--stdio-inherit-on-windows--powershell-input-hijack)
10. [Model capabilities 鈥?context window, reasoning, descriptors](#10-model-capabilities--context-window-reasoning-descriptors)
11. [Workflow process events and lifecycle controls](#11-workflow-process-events-and-lifecycle-controls-feature_229-v0750)
12. [Provider credential verification 鈥?`verifyProviderCredential`](#12-provider-credential-verification--verifyprovidercredential-feature_216-v0745)
13. [Inject your product's manual 鈥?`selfManual`](#13-inject-your-products-manual--selfmanual-feature_221-v0747)
14. [Media input artifacts 鈥?`@kodax-ai/kodax/media`](#14-media-input-artifacts--kodax-aikodaxmedia-feature_239-v0756)
...
> `await import(...)` 鈥?see 搂5 for the full recipe and bundler
> configuration.

---

## 1. MCP server management 鈥?`McpManager` runtime API
...
`execute` / `read` / `getPrompt`) 鈥?that's the substrate-facing API,
...
// Full catalog (tools + resources + prompts) 鈥?popout's "all capabilities" tab.
...
| `getServerLogs(id)` | `McpServerLogs` | `{ status, connect, lastError?, cachedAt? }`. Designed as the data source for a "Logs" pane. **v0.7.42 is intentionally conservative** 鈥?only last error + status are exposed. Future iterations may add a ring buffer; the field shape will extend, never break. |
...
| `provider()` | `McpCapabilityProvider` | Escape hatch 鈥?the underlying capability-provider for advanced uses. |
...
| `disabled` | `connect: 'disabled'` in config 鈥?runtime exists but won't be used. |

### Adding / removing servers

For server **config** CRUD (write to `~/.kodax/config.json`), use the
`@kodax-ai/kodax/repl` subpath 鈥?kept separate from `/mcp` to keep
...
// compatibility you may also use `type: 'http'` (v0.7.48+) 鈥?a config-layer
...
that swaps configs hot constructs a fresh manager 鈥?there is no
...
**dynamic context** 鈥?markdown can embed `!`shell-command`` tokens
...
this is rarely the right behavior 鈥?the host typically wants to
...
const context: SkillContext = {
  ...
  // v0.7.42 鈥?host hook for !`cmd` execution.
...
// banner 鈥?depending on your hook's throw behavior).
```

### Disabling dynamic context entirely

For maximum-safety hosts (e.g. a security-audit popout), set
`disableDynamicContext: true` 鈥?every `!`cmd`` token is replaced with
...
**Hosts should always set `executeDynamicContext`** 鈥?the legacy
...
|---|---|---|
| `IVariableResolver` (interface) | Type contract 鈥?`resolve(content): Promise<string>` | `@kodax-ai/kodax/skills` |
...
| `resolveSkillContent(content, args, context)` (top-level) | High-level convenience 鈥?constructs resolver internally + resolves a single skill body | `@kodax-ai/kodax/skills` |
...
directly and call `resolver.resolve(content)` themselves 鈥?KodaX's
...
## 3. Per-app data directory namespacing 鈥?`getAppDataDir`

### Why this exists

`~/.kodax/` is KodaX's own state directory 鈥?sessions, custom
...
(two extensions both writing `~/.kodax/cache/`) 鈥?and writing
...
- Reserved prefix: `kodax-*` (and the bare string `kodax`) 鈥?rejected
  to prevent host apps from squatting on names that look like KodaX's
  own subsystems.

| `appId` | Result |
|---|---|
| `space` | 鉁?`<KODAX_HOME>/apps/space/` |
| `kodax-space-helper` | 鉁?`Error: reserved appId prefix 'kodax-*'` |
| `MyApp` | 鉁?`Error: appId must match /^[a-z][a-z0-9-]{1,31}$/` |
| `a` | 鉁?`Error: appId must be 2-32 chars` |

### Interaction with `setAgentConfigHome`

`getAppDataDir` resolves `<KODAX_HOME>` via `getAgentConfigHome()` on
every call 鈥?so a host that called `setAgentConfigHome('/custom')`
...
|---|---|---|
| `@kodax-ai/kodax` | `runKodaX(opts, prompt)` | Blocking `Promise<KodaXResult>` 鈥?the original entry. |
...
| `@kodax-ai/kodax/coding` | `validateCustomProviderConfig(config)` | Same validator the SDK uses internally 鈥?no parallel schemas. |
...
// Mid-run mutators 鈥?applied on the next turn (CAP-055 per-turn re-resolution).
...
In a CommonJS context 鈥?Electron's main process (`vm.mainModule` runs
as CJS by default), legacy Webpack/esbuild configs with
`format: 'cjs'`, or any code path that ends up calling `require()` 鈥?
...
```ts
// 鉂?This breaks in CJS 鈥?esbuild / tsc transforms static `import`
...
// 鉁?Use dynamic import 鈥?Node resolves it via the ESM loader and
//    matches our `import` condition.
const { McpManager } = await import('@kodax-ai/kodax/mcp');
```

`import type { ... }` is **fine in CJS** 鈥?TypeScript / esbuild strip
...
// 鈫?compiles to nothing at runtime
```
...
|---|---|---|
| `/agent` | 0 | 鉁?feasible (no UI deps) |
| `/mcp` | 0 | 鉁?feasible (no UI deps) |
| `/skills` | 1 (`yaml`) | 鉂?|
| `/llm` | 2 (`@agentclientprotocol/sdk`, `partial-json`) | 鉂?|
| `/coding` | 4 (`yaml`, `tsx`, 鈥? | 鉂?|
| `/repl` | **21** (`ink`, `chalk`, `react`, `ansi-escapes`, 鈥? | 鉂?|
| root | 21 (same as `/repl`) | 鉂?|

Once a bundle inlines any ESM-only dependency, the bundle itself
cannot be a valid CJS module 鈥?`require()`ing it would synchronously
import an ESM dep, which Node refuses. `ink`, `chalk`, and most of
the modern terminal-UI ecosystem are ESM-only as of 2024鈥?026, with
...
```ts
// main.ts (or main.cjs) 鈥?Electron main process
...
async function bootKodaX() {
  ...
  // esbuild rewrite them 鈥?see "Bundler config" below).
...
// Wire to IPC handlers 鈥?your popout UI calls these.
...
Alternatively, mark only `@kodax-ai/kodax` as external 鈥?bundlers
...
synchronous `Error: ERR_REQUIRE_ESM` (Node 鈮?20) or
...
If your host environment cannot adopt `await import(...)` (rare 鈥?
...
build. We have not productized this yet 鈥?file an issue with your
...
- [ ] `package.json#type` in **your** project 鈥?if it's `"module"`,
      static `import` works; if absent / `"commonjs"`, you need
      dynamic `await import()`.
- [ ] Your bundler emit format 鈥?`format: 'cjs'` + static `import`
...
- [ ] Node version 鈮?20.19 (or 鈮?22 for the more permissive
      `--experimental-require-module` default).

---

## 6. Session persistence 鈥?wiring `runKodaX` to disk
...
await runKodaX(
  {
    ...
    session: { id: 's_my_chat', scope: 'user' },  // 鈫?session.id set
  },
  'reply with: ok',
);
// 鉁?Run completes, LLM streams, events fire 鈥?but
...
This is by design 鈥?the CLI ships its own storage wiring, and the
...
but `session.storage` is missing** 鈥?it points at this section.
...
await runKodaX(
  {
    ...
    session: {
      id: 's_my_chat',
      scope: 'user',
      storage,                  // 鈫?key 鈥?wire the storage instance
    },
  },
  'reply with: ok',
);
// 鉁?~/.kodax/sessions/s_my_chat.jsonl now exists after the run.
...
interface SessionManager {
  ...
  storage: FileSessionStorage;     // 鈫?NEW 鈥?pass into runKodaX
}
```
...
class MyDbSessionStorage implements KodaXSessionStorage {
  ...
  // (other methods 鈥?see the @kodax-ai/kodax/coding type for the full surface)
}
...
The SA / AMA loops are storage-implementation-agnostic 鈥?they just
call `storage.save(...)` at the terminal sites (success / error /
mid-flow / limit-reached). Storage failures are swallowed locally
with a `[SessionSnapshot] storage.save failed` `console.error` 鈥?
...
|---|---|---|
| Only need to save runs, no listing / reading UI | 鉁?| (also fine 鈥?`storage` field gives you the same instance) |
| Need a popout / sidebar showing past sessions | | 鉁?鈥?pairs read + write through one instance |
| Custom sessions dir for tests / tenants | (works, but you must repeat the dir option for each call) | 鉁?鈥?`{ sessionsDir }` once, both sides honor it |
| Want to mock storage for unit tests | 鉁?(inject mock implementing `KodaXSessionStorage`) | 鈥?|
...
- [ ] Is the `session.id` filesystem-safe? KodaX accepts any string
      but writes `<id>.jsonl` literally 鈥?IDs with `/`, `\`, or
...
build 鈥?verifying a bugfix, prototyping against an in-progress feature,
running your project's test suite against `main` 鈥?you can `npm link`
...
exports. `npm link` "just works" 鈥?no need to run `scripts/release.mjs`
first.
...
npm run build                                  # required 鈥?npm link resolves to dist/
...
inside KodaX require **re-running `npm run build`** 鈥?the link points
...
from the repo refuses 鈥?`scripts/release.mjs` is the only sanctioned
publish path, and it briefly toggles `private: false` (via try/finally)
just for the publish call. `"private"` does **not** block `npm link` 鈥?
it only gates `npm publish` 鈥?so the linked-build flow is unaffected.
...
## 8. User-authored agents 鈥?markdown loader + extension `registerAgent` (FEATURE_191, v0.7.43)
...
the extension id + agent name + verdict reason 鈥?the embedder sees
...
> **Source-aware variants** 鈥?`listConstructedAgentsWithSource()` /
...
> sub-barrel 鈥?they will be promoted to the top-level SDK entry when
...
the standard tool surface 鈥?pass `subagent_type` to
`dispatch_child_task`:
...
- [docs/test-guides/FEATURE_191_v0.7.43_TEST_GUIDE.md](test-guides/FEATURE_191_v0.7.43_TEST_GUIDE.md) 鈥?manual test recipes
- [docs/features/v0.7.43.md FEATURE_191](features/v0.7.43.md#feature_191-user-authored-custom-agents--markdown-loader--extension-registeragent--dispatch_child_task-bridge) 鈥?design + acceptance gates
- [docs/ADR.md ADR-035](ADR.md#adr-035-user-authored-custom-agents--markdown-loader--extension-registeragent--dispatch_child_task-bridge-feature_191-v0743) 鈥?architectural rationale

---

## 9. Electron + `stdio: 'inherit'` on Windows 鈥?PowerShell input hijack
...
cmd.exe) stops responding to keyboard input 鈥?characters don't echo,
...
鈥?the child shell never sees the parent stdin.
...
keystrokes 鈥?they cannot tell whether the upstream child has
...
const electron = spawn(electronBin, [appEntry], {
  stdio: ['ignore', 'inherit', 'inherit'], // 鈫?stdin: 'ignore', NOT 'inherit'
  shell: false,
});
```
...
"no state change 鉁?, the issue is in your spawn config 鈥?not in KodaX.
If any step shows a non-zero delta, file an issue with the probe output
and your Node / OS / SDK version.

---

## 10. Model capabilities 鈥?context window, reasoning, descriptors
...
shown next to each model 鈥?so the user can pick informed. Pre-v0.7.43,
...
throws if the relevant API key env var is unset 鈥?meaning a UI couldn't
...
All exports below come from `@kodax-ai/kodax/llm` (preferred) 鈥?also
...
import {
  ...
  // Unified dispatchers 鈥?built-in OR custom, transparent routing:
...
interface KodaXModelCapabilities {
  ...
  displayName: string;              // human label 鈥?falls back to model id
  ...
  maxOutputTokens?: number;         // per-turn max_tokens KodaX requests 鈥?see note below
...
- **What upstream advertises is often unreliable.** A 2026-05 probe
  ...
  endpoints return only `{id, object, owned_by, created}` 鈥?no
...
asks the model for 鈥?i.e. the actual size budget your turn gets. If
...
/ `getModelDescriptor()` instance methods still work 鈥?they're the
...
in the four-step cascade (compactionConfig override 鈫?per-model 鈫?
provider default 鈫?200K fallback) should still use
`resolveContextWindow(compactionConfig, provider, model)` from
`@kodax-ai/kodax/agent`. The new registry-layer getters are for the
*"list everything KodaX knows"* case 鈥?for picker UIs, comparison
...
the patch site 鈥?the new value flows to runtime (via `buildProviderConfig`) AND
...
and [`scripts/probe-ark-tokens.mjs`](../scripts/probe-ark-tokens.mjs) 鈥?
...
## 12. Provider credential verification 鈥?`verifyProviderCredential` (FEATURE_216, v0.7.45)

### Why

The original SDK exposed `provider.isConfigured()` (env-only check) and the full streaming surface (`provider.stream()` / `sideQuery()`). Neither fits the "test connection" UI use case: env check doesn't validate the key actually works against the upstream; streaming costs ~50鈥?00 tokens and several seconds. KodaX Space (and any third-party SDK consumer building a provider-settings UI) needs a lightweight server-validated check.

FEATURE_216 ships **`verifyProviderCredential(name, opts?)`** 鈥?never-throws, lightweight, per-provider-strategy.
...
if (result.ok) {
  ...
  console.log(`鉁?Verified in ${result.durationMs}ms (${result.approxTokensSpent} tokens)`);
} else {
  switch (result.error) {
    case 'unauthorized':  /* show "invalid key 鈥?auth failed" (covers 401/403 + kimi-code 400 special) */ break;
    ...
    case 'rate_limited':  /* 429 鈥?key is valid but throttled; suggest retry */ break;
...
- **Never throws** 鈥?every failure mode is captured in the returned `KodaXVerifyCredentialResult` envelope. Mirrors the `side-query.ts` pattern. Guarantee holds even for runtime-registered providers whose `verifyCredential()` override might throw (legacy 3rd-party extensions that predate FEATURE_216): the top-level helper wraps the call in try/catch and returns `error: 'unknown'`.
- **No ctor throw on missing env** 鈥?the helper short-circuits to `error: 'unconfigured'` BEFORE attempting to instantiate the provider class (which would call `getApiKey()` and throw).
- **Lightweight** 鈥?9 of the 12 verifiable providers run a **zero-token** primitive; the remaining 3 cost ~6鈥? tokens per call (~$0.00001 at typical rates).
- **Cancellable** 鈥?pass `opts.signal` (any `AbortSignal`); the helper distinguishes timeout vs parent-abort in the result.
- **Key redaction** 鈥?`result.message` redacts `sk-...` patterns before being surfaced, so an upstream error body that echoes the submitted key won't leak the fragment into a UI log or display.
...
- `protocol: 'anthropic'` 鈫?defaults to `count-tokens`
- `protocol: 'openai'` 鈫?defaults to `models-list`

Override with explicit `verifyStrategy` when the upstream needs a different primitive (e.g. an openai-compat gateway whose `/v1/models` is public 鈥?set `verifyStrategy: 'minimal-message'`):
...
- `protocol: 'openai'` + `verifyStrategy: 'count-tokens'` 鈫?throws (OpenAI protocol has no count_tokens endpoint).

### Model listing 鈥?`listProviderModels(name)`

Separate API for "model picker" UIs. Returns the static model list KodaX maintains in `provider-capabilities.json` (or the custom provider's `models` field). Always `source: 'static'` in v0.7.45 鈥?KodaX's curated list is more reliable than upstream `/v1/models` (which is noisy, includes deprecated entries, or 鈥?in zhipu's case 鈥?is publicly served regardless of auth).
...
Cli-bridge providers (`gemini-cli`, `codex-cli`) return their CLI binary's known model list 鈥?filled at SDK load via `cli-bridge-models.ts`.
...
- **Still tool-on-demand + bounded.** Nothing big is injected into the prompt 鈥?only the 鈮?50-token routing rule. Topics live in the registry and are returned one at a time when the model calls the tool, each capped at 4 KB. Drift-guarding your own topics (e.g. not referencing a removed setting) is your responsibility.

### Topic shape (`KodaXManualTopicInput`)

`{ id, title, summary, body }` required; `aliases?`, `nextTopics?`, `sources?` optional. Keep `body` short (a few lines) 鈥?it is a bounded on-demand answer, not a document.
...
## 14. Media input artifacts 鈥?`@kodax-ai/kodax/media` (FEATURE_239, v0.7.56)
...
// Pass the artifacts through `runKodaX` (or the `KodaXClient` constructor) 鈥?
