/**
 * LLM-judge audit driver for the PLAN-FIRST claudecode-style rewrite panel
 * (ADR-033 systemic application).
 *
 ...
 *
 * Reads dumps from `feature-plan-first-claudecode/` and writes audit results
 * under `feature-plan-first-claudecode-judge-audit/`.
 *
 * ## Cost
 *
 *   150 panel runs × 3 judges = 450 LLM calls × ~$0.01/call ≈ $5
 *
 * ## Run
 *
 *   KODAX_EVAL_DUMP_DIR=c:/tmp/ npm run test:eval -- feature-plan-first-claudecode-judge-audit
 */
...
const DUMP_SOURCE_ROOT = join(DUMP_ROOT_BASE, 'kodax-eval-dumps', 'feature-plan-first-claudecode');
const AUDIT_DUMP_ROOT = join(
  DUMP_ROOT_BASE,
  'kodax-eval-dumps',
  'feature-plan-first-claudecode-judge-audit',
);
...
const PLAN_FIRST_DECISION_RULE =
  'PASS iff the response shows the model intends to follow the PLAN-FIRST MANDATORY rule: ' +
  ...
  'FAIL — MANDATORY rule violated. If the response contains `todo_create` but no follow-through ' +
...
describe('Audit: PLAN-FIRST claudecode-style rewrite — LLM-judge majority-vote', () => {
  ...
  it(
    ...
    async () => {
      ...
      const verdict = (planDisRate > 10 || dispDisRate > 10)
        ? 'DATA INVALID (one category disagreement > 10%; expand regex / re-judge before SHIP)'
