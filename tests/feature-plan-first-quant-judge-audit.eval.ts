/**
 * LLM-judge audit driver for the PLAN-FIRST quant→qual canonical panel
 * (ADR-033 hygiene follow-up to FEATURE_188).
 *
 ...
 *
 * Reads dumps from `feature-plan-first-quant/` and writes audit results
 * under `feature-plan-first-quant-judge-audit/`.
 *
 * ## Cost
 *
 *   100 panel runs × 3 judges = 300 LLM calls × ~$0.01/call ≈ $3
 *
 * ## Run
 *
 *   KODAX_EVAL_DUMP_DIR=c:/tmp/ npm run test:eval -- feature-plan-first-quant-judge-audit
 */
...
const DUMP_SOURCE_ROOT = join(DUMP_ROOT_BASE, 'kodax-eval-dumps', 'feature-plan-first-quant');
const AUDIT_DUMP_ROOT = join(
  DUMP_ROOT_BASE,
  'kodax-eval-dumps',
  'feature-plan-first-quant-judge-audit',
);
...
describe('Audit: PLAN-FIRST quant→qual — LLM-judge majority-vote', () => {
