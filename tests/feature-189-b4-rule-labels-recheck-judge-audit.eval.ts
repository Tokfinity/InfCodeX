/**
 * LLM-judge audit driver for FEATURE_189 B.4 RECHECK pilot (2026-05-24).
 *
 ...
 *
 * Reads dumps from `feature-189-b4-rule-labels-recheck-pilot/` and
 * writes audit results under
 * `feature-189-b4-rule-labels-recheck-judge-audit/`.
 *
 * ## Cost
 *
 *   12 pilot runs × 3 judges × 2 categories = 72 LLM calls × ~$0.01/call ≈ $0.7
 *
 * ## Run
 *
 *   KODAX_EVAL_DUMP_DIR=c:/tmp/ npm run test:eval -- feature-189-b4-rule-labels-recheck-judge-audit
 */
...
describe('Audit: FEATURE_189 B.4 RECHECK pilot — LLM-judge majority-vote', () => {
    ...
    async () => {
      ...
      for (const caseId of CASE_IDS) {
        ...
        try {
          ...
          console.log(`[recheck-audit][${caseId}] RESUME-SKIP — dump exists`);
          continue;
        } catch {
          // proceed
        }
        ...
        try {
          ...
          // eslint-disable-next-line no-console
          console.log(
            `[recheck-audit][${caseId}] SKIP — ${err instanceof Error ? err.message : String(err)}`,
          );
          continue;
        }
      ...
      const finalLines: string[] = ['', '[recheck-audit][SUMMARY]'];
