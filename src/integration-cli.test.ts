...
beforeAll(async () => {
  ...
  configHome = path.join(mkdtempSync(path.join(os.tmpdir(), 'kodax-integration-cli-')), 'custom-config-home');
...
describe('integration CLI', () => {
  ...
  it('configures an external OAuth2 issuer for JWT access-token validation', async () => {
    await runCommand([
      ...
      '--oauth-metadata-url', 'https://identity.example.com/.well-known/oauth-authorization-server',
  ...
  it('manages outbound Agent activation without deleting its configuration', async () => {
    await runCommand([
      'a2a', 'add', 'managed', 'https://agents.example.com/card', '--disabled', '--no-test',
    ]);
  ...
  it('does not offer a non-persistent private-network override when adding an Agent', async () => {
    await expect(runCommand([
      'a2a', 'add', 'private-only', 'https://10.0.0.1/.well-known/agent-card.json',
      '--allow-private', '--no-test',
  ...
  it('requires an explicit stopped-daemon confirmation to migrate a non-empty v1 file', async () => {
    ...
    try {
      await expect(runCommand(['a2a', 'disable', 'legacy']))
        .rejects.toThrow(/explicit migration|a2a migrate/i);
      expect(readFileSync(file, 'utf8')).toBe(legacy);
      await expect(runCommand(['a2a', 'migrate']))
        .rejects.toThrow(/confirm-daemons-stopped/i);
      expect(readFileSync(file, 'utf8')).toBe(legacy);

      const output = JSON.parse(await runCommand([
        'a2a', 'migrate', '--confirm-daemons-stopped',
  ...
  it('fails closed for legacy or mismatched daemon ownership beside a non-.kodax config home', async () => {
    const [runtimeModule, hostModule, stateModule, transportModule] = await Promise.all([
    ...
    const paths = stateModule.resolveRuntimeDaemonPathsFromConfigHome(configHome, profile);
    ...
    const host = await hostModule.startRuntimeDaemonHost({
      runtime,
      paths,
      endpoint: transportModule.defaultRuntimeDaemonEndpoint(profile, path.dirname(configHome)),
      lock,
    });
    const file = path.join(configHome, 'integrations', 'a2a.json');
    mkdirSync(path.dirname(file), { recursive: true });
    ...
    try {
      await expect(runCommand(['a2a', 'disable', 'guarded']))
        .rejects.toThrow(/a2aConfigReconciler|safely apply A2A config/i);
      expect(readFileSync(file, 'utf8')).toBe(document);

      const idempotentMigration = JSON.parse(await runCommand(['a2a', 'migrate'])) as {
        readonly migrated: boolean;
        readonly version: number;
      };
      expect(idempotentMigration).toMatchObject({ migrated: false, version: 2 });
      expect(readFileSync(file, 'utf8')).toBe(document);

      const currentState = JSON.parse(readFileSync(paths.stateFile, 'utf8')) as Record<string, unknown>;
      const legacyState = { ...currentState };
      delete legacyState.configHome;
      writeFileSync(paths.stateFile, `${JSON.stringify(legacyState, null, 2)}\n`, 'utf8');
      await expect(runCommand(['a2a', 'disable', 'guarded']))
        .rejects.toThrow(/does not declare.*config home|restart.*current KodaX/i);
      expect(readFileSync(file, 'utf8')).toBe(document);
      ...
      await expect(runCommand(['a2a', 'remove', 'guarded']))
        .rejects.toThrow(/different.*config home|ambiguous ownership/i);
      expect(readFileSync(file, 'utf8')).toBe(document);
    } finally {
      await host.close();
      if (previous === undefined) rmSync(file, { force: true });
  ...
  it.each([
    { label: 'raw, data, and HTTP(S) artifact references', rawBytes: 32, includeReferences: true },
    { label: 'a task response above the 2 MiB metadata limit', rawBytes: 1_600_000, includeReferences: false },
  ])('allows $label through `a2a call` without fetching remote artifacts', async ({
    label, rawBytes, includeReferences,
  }) => {
    let baseUrl = '';
    const name = includeReferences ? 'artifact-policy-loopback' : 'large-artifact-loopback';
    ...
    const server = createServer(async (request, response) => {
      ...
      if (request.url === '/card') {
        response.end(JSON.stringify({
          ...
          description: 'Returns artifact references without materializing remote URLs.',
          version: '1.0.0',
          supportedInterfaces: [{
            url: `${baseUrl}/rpc`, protocolBinding: 'JSONRPC', protocolVersion: '1.0',
          }],
      ...
      if (payload.method === 'SendMessage') {
        response.end(JSON.stringify({
          ...
          result: {
            task: {
              id: `task-${name}`, contextId: `context-${name}`,
              status: { state: 'TASK_STATE_SUBMITTED' },
            },
          },
        }));
      ...
      const artifacts = [{
        artifactId: 'raw-artifact', name: 'report.bin',
        parts: [{ raw, mediaType: 'application/octet-stream', filename: 'report.bin' }],
      }, ...(includeReferences ? [{
        artifactId: 'data-artifact', name: 'summary.json',
        parts: [{ data, mediaType: 'application/json', filename: 'summary.json' }],
      }, {
        artifactId: 'http-artifact', name: 'remote-http.pdf',
        parts: [{ url: `${baseUrl}/artifact-must-not-be-fetched`, mediaType: 'application/pdf' }],
      }, {
        artifactId: 'https-artifact', name: 'remote-https.pdf',
        parts: [{ url: 'https://artifacts.example.com/report.pdf', mediaType: 'application/pdf' }],
      }] : [])];
      response.end(JSON.stringify({
        jsonrpc: '2.0', id: payload.id,
        result: {
    ...
    try {
      ...
      expect(result).toContain(`data:application/octet-stream;base64,${raw.slice(0, 16)}`);
      if (includeReferences) {
        const encodedData = Buffer.from(JSON.stringify(data), 'utf8').toString('base64');
        expect(result).toContain(`data:application/json;base64,${encodedData}`);
  ...
  it('obtains an external OAuth2 client-credentials token for a no-code A2A call', async () => {
    ...
    const server = createServer(async (request, response) => {
      ...
      if (request.url === '/token') {
        ...
        expect(Buffer.concat(chunks).toString('utf8')).toContain('grant_type=client_credentials');
        response.end(JSON.stringify({
          access_token: 'issued-access-token', token_type: 'Bearer', expires_in: 120,
        }));
        return;
      }
      rpcAuthorization = request.headers.authorization ?? '';
      const payload = JSON.parse(Buffer.concat(chunks).toString('utf8')) as { readonly id: string };
      response.end(JSON.stringify({
        jsonrpc: '2.0', id: payload.id,
        result: {
          message: {
            messageId: 'oauth-result', role: 'ROLE_AGENT',
    ...
    try {
      await runCommand([
        'a2a', 'add', 'oauth-loopback', `${baseUrl}/card`, '--no-test',
        '--oauth-scheme', 'enterprise',
        '--oauth-issuer', `${baseUrl}/`,
        '--oauth-token-url', `${baseUrl}/token`,
        '--oauth-client-id', 'kodax-cli',
        '--oauth-client-secret-env', 'CLI_A2A_CLIENT_SECRET',
        '--oauth-scope', 'a2a.invoke',
      ]);
      await expect(runCommand(['a2a', 'call', 'oauth-loopback', 'Prepare report']))
        .resolves.toContain('oauth completed');
      expect(tokenRequests).toBe(1);
      expect(rpcAuthorization).toBe('Bearer issued-access-token');
      expect(readFileSync(path.join(configHome, 'integrations', 'a2a.json'), 'utf8'))
        .not.toContain('local-client-secret');
      await runCommand(['a2a', 'remove', 'oauth-loopback']);
    } finally {
      if (previousSecret === undefined) delete process.env.CLI_A2A_CLIENT_SECRET;
