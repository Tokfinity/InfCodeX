...
export const BUILTIN_TOOL_DEFINITIONS: LocalToolDefinition[] = [
  ...
  },
  {
    name: 'run_workflow',
    description:
      "Author and run a multi-agent workflow inline: you write a small JavaScript orchestration script that runs as child agents under the workflow runtime. run_workflow is ASYNCHRONOUS — the call returns immediately with a task_id and the workflow runs in the background; its synthesized result arrives later as a <task-completed task_id=…> block, so after calling it you idle-yield (end your turn with no tool calls) or do other work rather than waiting inline, exactly like dispatch_child_task. Reach for this when independent agents working in parallel and then cross-checking each other beat a single linear pass — comparing several codebases or options, reviewing a diff from multiple independent angles and adversarially verifying each finding, or fanning many files out and synthesizing what they return. First investigate with your own tools (read / grep / glob) to find the real files, the actual sub-problems, and the shape of the data; then bake those concrete findings into the child prompts — exact paths, the specific dimensions to compare, a real outputSchema. A script that only says \"investigate X\" and re-delegates all the scouting to its children produces shallow, duplicated work, because each child starts from nothing; the value of the workflow is that its children start from what you already discovered. The run(wf, args) body coordinates agents through wf only: wf.runAgent({name, prompt, readOnly, phase?, outputSchema?}) returns the child's result, or null if it failed or was stopped (so you can .filter(Boolean) instead of guarding every call), and with outputSchema — a JSON-Schema subset of type/enum/required/properties/items/additionalProperties — the parsed validated object comes back on result.structured; wf.parallel([thunks]) runs thunks concurrently and nulls any that fail rather than rejecting; wf.pipeline(items, stage1, stage2) streams each item through the stages without waiting for the slowest; wf.synthesize({inputs, rubric}) folds results into one; wf.workflow(name, args) runs a saved or built-in workflow as a sub-step; a shared phase tag groups agents in the progress view. Patterns worth composing (these are examples, not a fixed menu): adversarially verify a finding by spawning a few independent agents asked to refute it and keeping it only if they cannot; run a judge panel by generating several independent attempts from different angles and synthesizing from the strongest; loop until dry by spawning finders until a couple of rounds surface nothing new rather than stopping at a fixed count; sweep multi-modally by pointing agents at one target through different routes (by file, by symbol, by test) so a single blind spot does not sink the search; end with a completeness critic that asks what is still missing. The most common shape for a review or audit is pipeline(findings, find, verify): fan independent reviewers out to surface findings, then pipe each finding straight into an independent verifier that tries to refute it, keeping only the ones it cannot dismiss and ending with .filter(Boolean). Weaving verification into the pipeline this way is what lets you skip a separate re-audit pass afterward, so set patterns to ['fan-out-and-synthesize', 'adversarial-verification'] when a script combines both shapes. Prefer dispatch_child_task for a single focused sub-task — a whole workflow's coordination overhead is wasted on one agent — and just read files yourself for a quick lookup.",
    input_schema: {
      type: 'object',
      properties: {
        manifest: {
