...
export function buildWorkerInstructions(
  ...
  const dispatchRules = [
    ...
    '- Continue useful local work after spawning. When an Agent result is required, call `wait_agent` with `return_on:"terminal"`; use its event sequence as the next `after_sequence` cursor. Do not poll ordinary progress events.',
    ...
    '- A terminal `wait_agent` response includes `terminalOutputs`; for a host-delivered `<agent-completed path="..." turn_id="..." state="completed">` block, its body is the authoritative terminal result. Use whichever arrives directly; use the inline result directly. Do not call `agent_output` speculatively or when either already supplied the result. Reserve `agent_output` for a terminal `list_agents` observation that supplies the exact Actor/Turn target, or for explicitly requested structured metadata that was not delivered.',
