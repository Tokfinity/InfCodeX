...
export function buildWorkerInstructions(
  ...
  const dispatchRules = [
    ...
    '- A terminal `wait_agent` response includes `terminalOutputs`; for a host-delivered `<agent-completed path="..." turn_id="..." state="completed">` block, its body is the authoritative terminal result. Use whichever arrives directly. Do not call `agent_output` speculatively or when either already supplied the result; reserve it for a terminal `list_agents` observation that supplies the exact Actor/Turn target, or for explicitly requested structured metadata that was not delivered.',
