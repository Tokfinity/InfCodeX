    ...
    case 'scout':
      return [
        ...
        [
          ...
          '',
          '  FEATURE_170 (v0.7.41) — MID-TASK REFINEMENT: once a plan exists, use',
          '  the per-item API rather than re-seeding the whole list:',
          '    * todo_create({content, activeForm?}) — INSERT ONE new step when the',
          '      plan needs an extra item but existing items must be preserved.',
          '      Store auto-mints the id.',
          '    * todo_update({id, content?, activeForm?, evaluator?, metadata?}) — patch one',
          '      step\'s description / present-continuous form / evaluator hint /',
          '      opaque metadata without changing its status.',
          ...
          '  NEVER call todo_update({op:"init", ...}) for mid-task insertion — that',
          '  wipes user-visible progress on items already completed.',
          '',
