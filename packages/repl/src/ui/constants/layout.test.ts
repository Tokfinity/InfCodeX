...
describe("help bar layout", () => {
  it("includes the SA/AMA/AMAW toggle shortcut in the help bar", () => {
    const helpText = buildHelpBarText();
    const helpSegments = buildHelpBarSegments();
    expect(helpText).toContain("Alt+M AMA/AMAW/SA");
    expect(helpSegments.some((segment) => segment.text === "Alt+M AMA/AMAW/SA")).toBe(true);
