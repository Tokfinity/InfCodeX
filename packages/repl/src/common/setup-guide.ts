...
export function renderSetupGuide(input: RenderSetupGuideInput = {}): string {
  ...
  return [
    ...
    '  The annotated core template includes `sandbox.envPass` for exact command-target',
    '  environment passthrough; setup never stores an environment-variable value.',
    '',
    ...
    '',
    'Sandbox containment (optional; permission decisions still work when it is unavailable):',
    ...
    '  Model-issued commands filter credential-shaped variables by default. Add exact names',
    '  such as GH_TOKEN to user-level `sandbox.envPass` only when command access is intended.',
    '  The default list is empty; NODE_OPTIONS and BASH_ENV remain blocked.',
    '  Restart KodaX (and any persistent daemon) after changing the list or host variables.',
    ...
    '  /mode [plan|accept-edits|auto]  Switch permission mode',
