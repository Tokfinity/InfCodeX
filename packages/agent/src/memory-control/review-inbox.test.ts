...
describe('FEATURE_260 episode review inbox', () => {
  ...
  it('writes a receipt before removing the pending entry', async () => {
    ...
    await expect(completeEpisodeReview(identity, 'review-1', ['proposal-1']))
  ...
  it('serializes new tenant root registration with a session-wide fence', async () => {
    ...
    while (!fenceQueued && Date.now() < queueDeadline) {
      try {
        fenceQueued = (await readdir(queuePath)).some((name) => name.startsWith('ticket-'));
      } catch {
        // The fair-lock queue may not exist until the fence reaches this root.
      }
      if (!fenceQueued) await new Promise<void>((resolve) => setTimeout(resolve, 10));
    }
    ...
    const registerTenantB = (async () => {
      ...
      const persisted = await persistPendingEpisodeReview(identityB, {
        ...digest(2),
  ...
  it('fences jobs outside the exact active branch even when their sequence is lower', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-exact-fence-'));
    const active = { ...digest(9), id: 'digest-active', reviewKey: 'review-active' };
    const sibling = {
      ...digest(1),
      id: 'digest-sibling',
    ...
    await persistPendingEpisodeReview({ ...identity, configHome: home }, active);
    ...
    const authority = JSON.parse(await readFile(path.join(
      ...
      readonly exactFences?: readonly { readonly retiredJobIds: readonly string[] }[];
    };
    expect(authority.exactFences?.some((fence) =>
      fence.retiredJobIds.includes(siblingReview.entry.jobId))).toBe(true);
  });

  it('prevalidates every tenant root before applying an exact session fence', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-fence-preflight-'));
    const tenantIds = ['tenant-preflight-a', 'tenant-preflight-b']
      .sort((left, right) =>
  ...
  it('does not delete a v2 envelope whose completed state has a mismatched identity', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-state-identity-'));
  ...
  it('holds session authority through a legacy v1 review effect and receipt', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-fence-'));
    const owner = { ...identity, configHome: home };
    const legacyDigest = {
      ...digest(1),
  ...
  it('retires an abandoned legacy v1 processing entry during a session fence', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-processing-'));
    const owner = { ...identity, configHome: home };
    const legacyDigest = {
      ...digest(1),
  ...
  it('preserves an active legacy processing entry when fencing a same-key pending entry', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-fence-collision-'));
    const owner = { ...identity, configHome: home };
    ...
    await writeFile(
      path.join(pendingDir, `${hashMemoryIdentityComponent('review', reviewKey)}.json`),
  ...
  it('does not let a stale legacy claim overwrite a same-key pending entry', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-stale-collision-'));
    const owner = { ...identity, configHome: home };
  ...
  it('does not replay a distinct stale legacy claim after the same key completes', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-receipt-gate-'));
    const owner = { ...identity, configHome: home };
    ...
    await expect(drainPendingEpisodeReviews(owner, options))
  ...
  it('removes a legacy pending residue protected by a committed receipt', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-pending-receipt-'));
    const owner = { ...identity, configHome: home };
    const reviewDigest = {
      ...digest(1),
    ...
    await completeEpisodeReview(owner, reviewDigest.reviewKey, ['proposal-completed']);
    await writeFile(pendingPath, `${JSON.stringify(entry)}\n`, 'utf8');
    let appliedEffects = 0;
  ...
  it('rejects malformed or mismatched receipts without deleting legacy work', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-invalid-receipt-'));
    const owner = { ...identity, configHome: home };
    const reviewDigest = {
      ...digest(1),
    ...
    for (const invalidReceipt of invalidReceipts) {
      await writeFile(receiptPath, `${JSON.stringify(invalidReceipt)}\n`, 'utf8');
      await expect(listPendingEpisodeReviews({
        configHome: home,
        tenantId: owner.tenantId,
      })).rejects.toThrow();
      await expect(readFile(pendingPath, 'utf8')).resolves.toContain(reviewDigest.id);
    }
  ...
  it('rejects empty legacy proposal ids without terminalizing the review', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-empty-id-'));
    const owner = { ...identity, configHome: home };
    const reviewDigest = {
      ...digest(1),
    ...
    await expect(completeEpisodeReview(owner, reviewDigest.reviewKey, ['']))
  ...
  it('binds legacy completion to its owner and keeps the first receipt immutable', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-owner-'));
    const owner = { ...identity, configHome: home };
    const intruder = {
      ...owner,
      agentId: 'agent-b',
    ...
    await writeFile(foreignReceiptPath, `${JSON.stringify({
      version: 1,
      reviewKey: reviewDigest.reviewKey,
      ownerAgentHash: hashMemoryIdentityComponent('agent', intruder.agentId),
      ownerProjectHash: hashMemoryIdentityComponent('project', intruder.projectId),
      proposalIds: ['proposal-first'],
      completedAt: '2026-07-12T00:01:00.000Z',
    })}\n`, 'utf8');
    await expect(completeEpisodeReview(owner, reviewDigest.reviewKey, ['proposal-first']))
      .rejects.toThrow('review owner identity mismatch');
    await rm(foreignReceiptPath, { force: true });

    await expect(completeEpisodeReview(intruder, reviewDigest.reviewKey, ['proposal-intruder']))
      .rejects.toThrow('review owner identity mismatch');
    await expect(readFile(pendingPath, 'utf8')).resolves.toContain(reviewDigest.id);

    const first = await completeEpisodeReview(
      owner,
      reviewDigest.reviewKey,
      ['proposal-first'],
    );
    const firstReceipt = await readFile(first.receiptPath, 'utf8');
    await expect(completeEpisodeReview(owner, reviewDigest.reviewKey, ['proposal-first']))
      .resolves.toEqual(first);
    await expect(completeEpisodeReview(owner, reviewDigest.reviewKey, ['proposal-replay']))
      .rejects.toThrow('review receipt payload mismatch');
    await expect(completeEpisodeReview(intruder, reviewDigest.reviewKey, ['proposal-first']))
      .rejects.toThrow('review owner identity mismatch');
    expect(await readFile(first.receiptPath, 'utf8')).toBe(firstReceipt);
    expect(JSON.parse(firstReceipt)).toMatchObject({ proposalIds: ['proposal-first'] });
  });

  it('checks legacy ownership before cleaning an ownerless receipt residue', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-inbox-v1-old-receipt-'));
    const owner = { ...identity, configHome: home };
    ...
    await expect(completeEpisodeReview(intruder, reviewDigest.reviewKey, ['proposal-old']))
      .rejects.toThrow('review owner identity mismatch');
    await expect(readFile(pendingPath, 'utf8')).resolves.toContain(reviewDigest.id);
    await expect(completeEpisodeReview(owner, reviewDigest.reviewKey, ['proposal-old']))
...
describe('FEATURE_263 fenced episode review protocol', () => {
  ...
  it('holds the branch fence through owner-session digest persistence', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-owner-atomic-'));
    setAgentConfigHome(home);
    const epoch = await captureEpisodeReviewBranchEpoch(identity);
  ...
  it('persists v2 work and freezes one immutable input on first claim', async () => {
    ...
    const first = await freezeEpisodeReviewInput(identity, claim, {
      evidence: { digest: digest(), exactSkillSnapshot: null, priorDigests: [] },
    ...
    const second = await freezeEpisodeReviewInput(identity, claim, {
  ...
  it('preserves __proto__ keys in the exact frozen evidence bytes', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-proto-evidence-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    expect(checkpoint.evidenceBytes)
      .toBe('{"__proto__":{"critical":"included"},"alpha":1}');
  });
  ...
  it('rejects a stale claim after a newer lease epoch is issued', async () => {
    ...
    if (first === undefined || second === undefined) throw new Error('expected both review claims');

    await expect(commitEpisodeReviewDecision(identity, first, {
      inputHash: 'a'.repeat(64),
      memoryProposalIds: [],
    }, new Date('2026-07-12T00:01:02.000Z'))).rejects.toThrow('review claim is no longer authoritative');
  ...
  it('commits one decision and idempotent action receipts under the live lease', async () => {
    ...
    await expect(commitEpisodeReviewDecision(identity, claim, {
      inputHash: input.evidenceHash,
      memoryProposalIds: ['different'],
    }, new Date('2026-07-12T00:01:01.000Z'))).resolves.toEqual(decision);
    const first = await commitEpisodeReviewAction(identity, claim, {
      actionId: `${decision.decisionId}:memory`,
      decisionId: decision.decisionId,
      carrier: 'memory',
      resultRefs: ['proposal-1'],
    }, new Date('2026-07-12T00:01:02.000Z'));
    const second = await commitEpisodeReviewAction(identity, claim, {
      actionId: `${decision.decisionId}:memory`,
      decisionId: decision.decisionId,
      carrier: 'memory',
      resultRefs: ['ignored'],
    ...
    expect(second).toEqual(first);
    await expect(commitEpisodeReviewAction(identity, claim, {
      actionId: `${decision.decisionId}:skill`,
      decisionId: decision.decisionId,
      carrier: 'skill',
      resultRefs: [''],
    }, new Date('2026-07-12T00:01:02.000Z'))).rejects.toThrow('resultRefs');
    await expect(commitEpisodeReviewAction(identity, claim, {
      actionId: `${decision.decisionId}:skill`,
      decisionId: decision.decisionId,
      carrier: 'skill',
      resultRefs: ['skill-record'],
    }, new Date('2026-07-12T00:01:02.000Z'))).rejects.toThrow('not required');
    await expect(commitEpisodeReviewAction(identity, claim, {
      actionId: `${decision.decisionId}:bogus`,
      decisionId: decision.decisionId,
      carrier: 'bogus' as 'memory',
      resultRefs: ['proposal-1'],
    }, new Date('2026-07-12T00:01:02.000Z'))).rejects.toThrow('carrier');
  });

  it('rejects empty v2 proposal ids before decision or completion state changes', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-empty-proposal-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    await expect(completeFencedEpisodeReview(identity, claim, ['']))
      .rejects.toThrow('proposalIds');
  ...
  it('rejects a conflicting terminal receipt without completing the v2 job', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-receipt-conflict-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    await expect(completeFencedEpisodeReview(identity, claim, ['proposal-first']))
      .rejects.toThrow('review receipt payload mismatch');
    expect((await inspectEpisodeReviewJob(identity, claim.jobId))?.state.status)
      .toBe('decided');
    expect(JSON.parse(await readFile(receiptPath, 'utf8')))
      .toMatchObject({ proposalIds: ['proposal-stale'] });
  });

  it('does not complete a v2 job before its decision and action receipts exist', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-premature-completion-'));
    setAgentConfigHome(home);
    const persisted = await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    expect((await inspectEpisodeReviewJob(identity, claim.jobId))?.state.status)
      .toBe('processing');
    await expect(readFile(persisted.path, 'utf8')).resolves.toContain(claim.jobId);
  });

  it('completes only after every carrier pinned by the decision has a receipt', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-required-carriers-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    await expect(completeFencedEpisodeReview(identity, claim, ['proposal-memory']))
  ...
  it('rejects a frozen input whose durable identity or evidence is forged', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-input-identity-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    await expect(readFile(path.join(jobRoot, 'review-input.json'), 'utf8'))
  ...
  it('rejects a decision whose durable identity does not match its job and input', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-decision-identity-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    await writeFile(path.join(jobRoot, 'decision.json'), `${JSON.stringify({
      version: 1,
      jobId: 'wrong-job',
    ...
    await expect(inspectEpisodeReviewJob(identity, claim.jobId))
      .rejects.toThrow('review decision identity mismatch');
    const state = JSON.parse(
      await readFile(path.join(jobRoot, 'state.json'), 'utf8'),
    ) as Record<string, unknown>;
  ...
  it('rejects an action receipt whose durable identity does not match its path', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-action-identity-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    await writeFile(
      path.join(actionsRoot, `${hashMemoryIdentityComponent('review', actionId)}.json`),
      `${JSON.stringify({
        version: 1,
        jobId: 'wrong-job',
  ...
  it('counts only provider failures and stops after the fourth failed attempt', async () => {
    ...
    expect(await claimEpisodeReview(identity, 'review-1', {
  ...
  it('resumes legacy v2 decisions with valid optional Skill receipts', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-legacy-carriers-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
    ...
    await expect(inspectEpisodeReviewJob(identity, claim.jobId)).resolves.toMatchObject({
  ...
  it('rejects an invalid carrier before invoking its external effect', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-invalid-carrier-'));
    setAgentConfigHome(home);
    const persisted = await persistPendingEpisodeReview(identity, digest());
    let effects = 0;
    ...
    expect((await inspectEpisodeReviewJob(identity, persisted.entry.jobId))?.actions)
      .toEqual([]);
  });

  it('rejects duplicate carriers before invoking either external effect', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-duplicate-carrier-'));
    setAgentConfigHome(home);
    const persisted = await persistPendingEpisodeReview(identity, digest());
    let effects = 0;
  ...
  it('defers owner/provider unavailability without consuming provider attempts', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-unavailable-'));
    setAgentConfigHome(home);
    await persistPendingEpisodeReview(identity, digest());
    const claim = await claimEpisodeReview(identity, 'review-1');
    if (claim === undefined) throw new Error('expected review claim');
  ...
  it('tracks carrier apply failures separately and never spends provider attempts', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-apply-retry-'));
    setAgentConfigHome(home);
    const persisted = await persistPendingEpisodeReview(identity, digest());
  ...
  it('keeps independent Memory and Skill retry budgets', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-carrier-budget-'));
    setAgentConfigHome(home);
    const persisted = await persistPendingEpisodeReview(identity, digest());
    const attempts = [
  ...
  it('skips attention and backoff jobs without spending the drain entry budget', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-drain-starvation-'));
    setAgentConfigHome(home);
  ...
  it('enforces the provider deadline when the reviewer ignores its AbortSignal', async () => {
    vi.useFakeTimers();
    try {
      home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-hard-timeout-'));
      setAgentConfigHome(home);
      await persistPendingEpisodeReview(identity, digest());
      ...
      await expect(draining).resolves.toMatchObject({
        failed: 1,
        failures: [{ reviewKey: 'review-1', error: 'episode reviewer timed out after 90000ms' }],
      });
      expect((await inspectEpisodeReviewJob(identity, 'review-1'))?.state)
  ...
  it('delivers completion only after all action receipts and retries a failed delivery', async () => {
    ...
    try {
      home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-completion-hook-'));
      setAgentConfigHome(home);
      await persistPendingEpisodeReview(identity, digest());
      ...
      const options = {
        ...
        applyV2Action: async (
          _entry: Parameters<NonNullable<EpisodeReviewDrainOptions['applyV2Action']>>[0],
          _decision: Parameters<NonNullable<EpisodeReviewDrainOptions['applyV2Action']>>[1],
          carrier: 'memory' | 'skill',
          _claim: Parameters<NonNullable<EpisodeReviewDrainOptions['applyV2Action']>>[3],
          commit: Parameters<NonNullable<EpisodeReviewDrainOptions['applyV2Action']>>[4],
        ) => {
          applyCalls += 1;
        ...
        onV2Completed: async (_entry: PendingEpisodeReviewV2) => {
          ...
          if (completionCalls === 1) throw new Error('session write interrupted');
        },
      };

      expect((await drainPendingEpisodeReviews(identity, options)).failed).toBe(1);
      expect(await inspectEpisodeReviewJob(identity, 'review-1')).toMatchObject({
        state: { status: 'pending', completionAttempts: 1 },
        actions: [{ carrier: 'memory' }, { carrier: 'skill' }],
      });
      await vi.advanceTimersByTimeAsync(60_000);
      expect((await drainPendingEpisodeReviews(identity, options)).reviewed).toBe(1);
  ...
  it('charges terminal receipt failures to completion without an optional hook', async () => {
    ...
    vi.setSystemTime(new Date('2026-07-12T00:01:00.000Z'));
    try {
      home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-terminal-receipt-'));
      setAgentConfigHome(home);
      const pending = await persistPendingEpisodeReview(identity, digest());
      const receiptsPath = path.join(path.dirname(path.dirname(pending.path)), 'receipts');
      ...
      expect((await drainPendingEpisodeReviews(identity, options)).failed).toBe(1);
      expect(await inspectEpisodeReviewJob(identity, pending.entry.jobId)).toMatchObject({
        state: {
          status: 'pending',
  ...
  it('ignores and removes a pending residue whose canonical job is already completed', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-completed-residue-'));
    setAgentConfigHome(home);
    const pending = await persistPendingEpisodeReview(identity, digest());
    ...
    await expect(listPendingEpisodeReviews({ tenantId: identity.tenantId }))
      .resolves.toEqual([]);
    await expect(readFile(pending.path, 'utf8')).rejects.toMatchObject({ code: 'ENOENT' });
  });

  it('writes distinct inbox receipts for distinct branch jobs with the same review key', async () => {
    home = await mkdtemp(path.join(os.tmpdir(), 'kodax-review-v2-distinct-receipts-'));
    setAgentConfigHome(home);
    const first = await persistPendingEpisodeReview(identity, digest());
    ...
    const receiptFiles = (await readdir(receiptsRoot, { recursive: true }))
      .filter((file) => file.includes(`receipts${path.sep}`) && file.endsWith('.json'))
      .map((file) => path.join(receiptsRoot, file));
    expect(receiptFiles).toHaveLength(2);
    const receipts = await Promise.all(
      receiptFiles.map(async (file) => JSON.parse(await readFile(file, 'utf8')) as {
        jobId?: string;
      }),
    );
